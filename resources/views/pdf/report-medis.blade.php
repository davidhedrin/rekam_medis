...
<body>
          ...
          <td style="padding-right: 25px">
            <table>
              <tbody>
                <tr>
                  <td colspan="3">
                    <small><strong><u>Informasi Report</u></strong></small>
                  </td>
                </tr>
                <tr>
                  <td><small>Dari</small></td>
                  <td><small>:</small></td>
                  <td><small>{{ $startDateSearch }}</small></td>
                </tr>
                <tr>
                  <td><small>Sampai</small></td>
                  <td><small>:</small></td>
                  <td><small>{{ $endDateSearch }}</small></td>
                </tr>
                <tr>
                  <td><small>Total Rekaman</small></td>
                  <td><small>:</small></td>
                  <td><small>{{ count($data) }} Baris</small></td>
                </tr>
              </tbody>
            </table>
          </td>
  ...
  <div>
    <small><strong><u>Report Rekam Medis</u></strong></small>
  </div>
  <div style="margin-bottom: 30px">
    <table class="table table-striped">
      ...
      <tbody>
        @forelse ($data as $index => $rc)
        <tr>
          <th><small>{{ $index+1 }}</small></th>
          <td><small>{{ $rc['created_at'] }}</small></td>
          <td><small>
            {{ $rc['master_record']['patient_name'] }}.
            <div>
            {{ $rc['master_record']['patient']['gender'] }}
            </div>
          </small></td>
        ...
        @empty
        <tr>
          <td colspan="8" style="text-align: center"><small><i>Belum ada riawayat terdaftar!</i></small></td>
        </tr>
        @endforelse
      </tbody>
    </table>
